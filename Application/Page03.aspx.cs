...
public partial class Application_Page03 : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (User.IsInRole("Call Center Agents"))
        {
            ...
            divTipsAccepted.Visible = true;
            divTipsAcceptedAgent.Visible = true;
        }
    }
    ...
    protected bool Save_All()
    {
        ...
        {
            rtrn = true;
            if (tbMerchantInitials03.Text.Length > 0) { Save_Initials(); }
            Update_Completed();
        }
        catch (Exception ex)
        {
    ...
    protected void getApplicationTips()
    {
        #region SQL Connection
        using (SqlConnection con = new SqlConnection(Custom.connStr))
        {
...
[as].[applicationid]
,[as].[acceptstips]
FROM [dbo].[application_site] [as] WITH(NOLOCK)
WHERE [as].[applicationid] = @sp_applicationid
                            ";
                cmdText += "\r";
                #endregion Build cmdText
                #region SQL Command Config
                ...
                using (SqlDataReader sqlRdr = cmd.ExecuteReader())
                {
                    if (sqlRdr.HasRows)
                    {
                        while (sqlRdr.Read())
                        {
                            if (sqlRdr["acceptstips"].ToString() == "True")
                            {
                                divTipsAccepted.Visible = true;
                            }
...
FROM [dbo].[application_signatures] [as] WITH(NOLOCK) WHERE [as].[applicationid] = @sp_applicationid
                ...
                using (SqlDataReader sqlRdr = cmd.ExecuteReader())
                {
                    if (sqlRdr.HasRows)
                    {
                        while (sqlRdr.Read())
                        {
                            tbMerchantInitials03.Text = sqlRdr["initialpart3"].ToString();
                        }
                    }
                    else
                    {
    ...
    protected void Save_Initials()
    {
        ...
        #region Field Values
        // table _signature
        var sp_initial = (tbMerchantInitials03.Text.Length > 0) ? tbMerchantInitials03.Text : null;
